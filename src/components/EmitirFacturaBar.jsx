...
export default function EmitirFacturaBar({ selectedCount, selectedVentas = [], onEmitir, onClear, onExport, onBulkDelete, onBulkRetry, onBulkAnular }) {
      ...
      <div className="flex items-center gap-3 bg-[#121212] border border-white/10 rounded-2xl px-5 py-3 shadow-2xl shadow-black/40">
        ...
        {hasFacturados && onBulkAnular && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-coral hover:bg-red-600 text-white text-xs font-bold uppercase tracking-wider transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-coral/25 cursor-pointer"
          >
            <RotateCcw size={14} className="rotate-[-45deg]" />
